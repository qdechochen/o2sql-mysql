...
class O2sqlMysql extends O2sql {
  ...
  async onExecuteHandler(command, client) {
    ...
    const { rows, insertId, affectedRows } = await this.query(text, values, client);
    ...
    return new Promise((resolve, reject) => {
      (client || this.pool).query(text, values, (error, results, fields) => {
        if (error) {
          ...
          resolve({ ...Array.isArray(results) ? { rows: results } : { ...results }, fields, });
        }
      });
    });
  }
          ...
          transitionClient.commit(err => {
            if (err) {
              transitionClient.rollback(() => {
                this._log('TRANSACTION ROLLBACK.............', err);
              });
              return reject(err);
            }
